...
if(TensorRT_INCLUDE_DIR AND EXISTS "${TensorRT_INCLUDE_DIR}/NvInferVersion.h")
    file(STRINGS "${TensorRT_INCLUDE_DIR}/NvInferVersion.h" _trt_ver_lines REGEX "^#define[ \t]+(NV_TENSORRT|TRT)_[A-Za-z0-9_]+[ \t]+")

    function(_tensorrt_read_define out_var define_name)
        foreach(_trt_ver_line IN LISTS _trt_ver_lines)
            if(_trt_ver_line MATCHES "^#define[ \t]+${define_name}[ \t]+([^ \t/]+)")
                set(_trt_define_value "${CMAKE_MATCH_1}")
                break()
            endif()
        endforeach()

        if(_trt_define_value MATCHES "^[0-9]+$")
            set(${out_var} "${_trt_define_value}" PARENT_SCOPE)
        elseif(DEFINED _trt_define_value)
            _tensorrt_read_define(_trt_resolved_value "${_trt_define_value}")
            set(${out_var} "${_trt_resolved_value}" PARENT_SCOPE)
        endif()
    endfunction()

    _tensorrt_read_define(TensorRT_VERSION_MAJOR NV_TENSORRT_MAJOR)
    _tensorrt_read_define(TensorRT_VERSION_MINOR NV_TENSORRT_MINOR)
    _tensorrt_read_define(TensorRT_VERSION_PATCH NV_TENSORRT_PATCH)
